...
TEST(CapsuleTest, GetVolume) {
    ...
    EXPECT_TRUE(FloatEqual(volume1, Real(0.25 * M_PI + (4.0/3.0) * M_PI * 0.125)));
    ...
    EXPECT_TRUE(FloatEqual(volume2, Real(16 * M_PI + (4.0/3.0) * M_PI * 8)));
    
    // 测试零高度胶囊体（退化为球体）
    Capsule capsule3(Real(0.0), Real(1.0)); // 高度0，半径1
    Real volume3 = capsule3.GetVolume();
    // 球体体积：(4/3) * π * r³ = (4/3) * π * 1³ = 4.1888π
    EXPECT_TRUE(FloatEqual(volume3, Real((4.0/3.0) * M_PI * 1.0)));
}
