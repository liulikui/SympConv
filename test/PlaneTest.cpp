...
TEST(PlaneTest, DefaultConstructor) {
    Plane plane;
    EXPECT_TRUE(SympConv::RealEqual(plane.x, Real(0.0)));
    EXPECT_TRUE(SympConv::RealEqual(plane.y, Real(0.0)));
    EXPECT_TRUE(SympConv::RealEqual(plane.z, Real(0.0)));
    EXPECT_TRUE(SympConv::RealEqual(plane.w, Real(0.0)));
}

TEST(PlaneTest, ParameterizedConstructor) {
    Plane plane(Real(1.0), Real(0.0), Real(0.0), Real(-1.0));
    EXPECT_TRUE(SympConv::RealEqual(plane.x, Real(1.0)));
    EXPECT_TRUE(SympConv::RealEqual(plane.y, Real(0.0)));
    EXPECT_TRUE(SympConv::RealEqual(plane.z, Real(0.0)));
    EXPECT_TRUE(SympConv::RealEqual(plane.w, Real(-1.0)));
}
...
TEST(PlaneTest, NormalAndPointConstructor) {
    ...
    EXPECT_TRUE(SympConv::RealEqual(plane.x, Real(0.0)));
    EXPECT_TRUE(SympConv::RealEqual(plane.y, Real(1.0)));
    EXPECT_TRUE(SympConv::RealEqual(plane.z, Real(0.0)));
    EXPECT_TRUE(SympConv::RealEqual(plane.w, Real(-5.0)));
}
...
TEST(PlaneTest, ThreePointsConstructor) {
    ...
    EXPECT_TRUE(SympConv::RealEqual(plane.x, Real(0.0)));
    EXPECT_TRUE(SympConv::RealEqual(plane.y, Real(0.0)));
    EXPECT_TRUE(SympConv::RealEqual(plane.z, Real(1.0)));
    EXPECT_TRUE(SympConv::RealEqual(plane.w, Real(0.0)));
}
...
TEST(PlaneTest, DistanceTo) {
    ...
    EXPECT_TRUE(SympConv::RealEqual(plane.DistanceTo(point1), Real(0.0)));
    
    Vec3 point2(Real(0.0), Real(6.0), Real(0.0)); // 在平面上方
    EXPECT_TRUE(SympConv::RealEqual(plane.DistanceTo(point2), Real(1.0)));
    
    Vec3 point3(Real(0.0), Real(4.0), Real(0.0)); // 在平面下方
    EXPECT_TRUE(SympConv::RealEqual(plane.DistanceTo(point3), Real(-1.0)));
}
...
TEST(PlaneTest, GetNormal) {
    ...
    EXPECT_TRUE(SympConv::RealEqual(normal.x, Real(1.0)));
    EXPECT_TRUE(SympConv::RealEqual(normal.y, Real(2.0)));
    EXPECT_TRUE(SympConv::RealEqual(normal.z, Real(3.0)));
}
...
TEST(PlaneTest, Normalize) {
    ...
    EXPECT_TRUE(SympConv::RealEqual(normalized.x, Real(1.0)));
    EXPECT_TRUE(SympConv::RealEqual(normalized.y, Real(0.0)));
    EXPECT_TRUE(SympConv::RealEqual(normalized.z, Real(0.0)));
    EXPECT_TRUE(SympConv::RealEqual(normalized.w, Real(-1.0)));
}
