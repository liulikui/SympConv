...
TEST(SphereShapeTest, RayCast) {
    // 创建一个半径为1的球体，中心在原点
    SphereShape sphereShape(1.0);
    
    // 测试从正面命中球体
    Ray ray1(Vec3(2, 0, 0), Vec3(-1, 0, 0), 10.0);
    RayCastResult result1;
    EXPECT_TRUE(sphereShape.RayCast(ray1, result1));
    ...
    EXPECT_TRUE(sphereShape.RayCast(ray2, result2));
