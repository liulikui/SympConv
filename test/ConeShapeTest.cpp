...
TEST(ConeShapeTest, GetLocalSupport) {
    ...
    EXPECT_TRUE(FloatEqual(support_y_pos.y, Real(2.0))); // 顶点
    ...
    EXPECT_TRUE(FloatEqual(support_z_pos.z, Real(1.0))); // 底面边缘
    
    // 测试负X方向
    Vec3 dir_x_neg(-1, 0, 0);
    Vec3 support_x_neg = shape.GetLocalSupport(dir_x_neg);
    EXPECT_TRUE(FloatEqual(support_x_neg.x, Real(-1.0))); // 底面边缘
    ...
    EXPECT_TRUE(FloatEqual(support_z_neg.z, Real(-1.0))); // 底面边缘
    
    // 测试斜向方向（X正，Y正，Z正）
    Vec3 dir_diagonal(1, 1, 1);
    Vec3 support_diagonal = shape.GetLocalSupport(dir_diagonal);
    EXPECT_TRUE(FloatEqual(support_diagonal.x, Real(0.0)));
    EXPECT_TRUE(FloatEqual(support_diagonal.y, Real(2.0))); // 顶点
    ...
    EXPECT_TRUE(FloatEqual(support_diagonal_neg_y.y, Real(0.0)));
...
TEST(ConeShapeTest, GetLocalSupport_DifferentSize) {
    ...
    Vec3 support_y_pos = shape.GetLocalSupport(dir_y_pos);
    EXPECT_TRUE(FloatEqual(support_y_pos.x, Real(0.0)));
    EXPECT_TRUE(FloatEqual(support_y_pos.y, Real(4.0))); // 顶点
    EXPECT_TRUE(FloatEqual(support_y_pos.z, Real(0.0)));
    
    // 测试正X方向
    Vec3 dir_x_pos(1, 0, 0);
    Vec3 support_x_pos = shape.GetLocalSupport(dir_x_pos);
    EXPECT_TRUE(FloatEqual(support_x_pos.x, Real(2.0))); // 底面边缘
    EXPECT_TRUE(FloatEqual(support_x_pos.y, Real(0.0)));
    EXPECT_TRUE(FloatEqual(support_x_pos.z, Real(0.0)));
...
TEST(ConeShapeTest, GetLocalBounds_DifferentSize) {
    ...
    EXPECT_TRUE(FloatEqual(bounds.mMax.y, Real(4.0))); // 高度
    EXPECT_TRUE(FloatEqual(bounds.mMax.z, Real(2.0)));
    
    // 验证AABB的中心
    Vec3 center = bounds.GetCenter();
    EXPECT_TRUE(FloatEqual(center.x, Real(0.0)));
    EXPECT_TRUE(FloatEqual(center.y, Real(2.0))); // 高度的一半
    EXPECT_TRUE(FloatEqual(center.z, Real(0.0)));
    
    // 验证AABB的大小
    Vec3 size = bounds.GetSize();
    EXPECT_TRUE(FloatEqual(size.x, Real(4.0))); // 直径
    EXPECT_TRUE(FloatEqual(size.y, Real(4.0))); // 高度
